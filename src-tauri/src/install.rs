...
use dirs::{data_local_dir, executable_dir, desktop_dir};
use reqwest::Client;
...
impl Installer {
    ...
    pub async fn install_tools(&self, client: Client, progress: &mut u64, progress_tool: impl Fn(String, u64)) {
        // download chunks
        for tool in TOOLS.iter() {
            ...
            let res = client
                ...
                .send()
                .await
                .or(Err(format!("Failed to GET from '{}'", &url))).unwrap();
            let total_size = res
                .content_length()
    ...
    pub async fn install(&self, progress_install: impl Fn(String, u64)) {
        ...
        if self.desktop_shortcut || self.menu_shortcut {
            generate_shortcut(self.version.clone(), self.description.clone(), self.desktop_shortcut, self.menu_shortcut);
